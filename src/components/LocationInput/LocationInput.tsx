...
export interface LocationInputProps {
  defaultValue: GhatType
  onChange?: (value: GhatType) => void
  onInputDone?: (value: GhatType) => void
  placeHolder?: string
  desc?: string
  className?: string
  autoFocus?: boolean
...
const LocationInput: FC<LocationInputProps> = ({
  ...
  const [searches, setSearches] = useState<GhatType[]>([])
  ...
  useEffect(() => {
    ...
    if (value.title !== '') {
      setSearches(
        searches
          .filter(search =>
            search.title.toLowerCase().includes(value.title.toLowerCase()),
          )
          .slice(0, 5),
      )
    }
  ...
  const handleSelectLocation = (item: GhatType) => {
    setValue(item)
  ...
  const renderRecentSearches = () => {
    const recentSearches: GhatType[] = JSON.parse(
      ...
      >
        ...
        <div className="flex-grow">
          <input
            ...
            onChange={e => {
              if (value.title.length > e.currentTarget.value.length)
                setSearches(ghats || [])
              setValue({
                _id: '',
